...
download.file(url = "https://github.com/carpentries-incubator/bioc-intro/raw/main/episodes/ datos/rnaseq.csv",
              destfile = "datos/rnaseq.csv")
```
...
## cargar los paquetes tidyverse, incl. dplyr
...
BiocManager::instalar("tidyverse")
```
...
```{r, purl=TRUE}
seleccionar (arn, gen, muestra, tejido, expresión)
```
...
```{r, purl=TRUE}
seleccionar (arn, -tejido, -organismo)
```
...
```{r, purl=TRUE}
filter(rna, sexo == "Masculino")
filter(rna, sex == "Masculino" & infección == "No infectado")
```
...
```{r, purl=TRUE}
filtro (genes, is.na (hsapiens_homolog_associated_gene_name))
```
...
```{r, purl=TRUE}
rna2 <- filter(rna, sexo == "Masculino")
rna3 <- select(rna2, gen, muestra, tejido, expresión)
rna3
```
...
```{r, purl=TRUE}
rna3 <- select(filtro(rna, sexo == "Masculino"), gen, muestra, tejido, expresión)
rna3
```
...
```{r, purl=TRUE}
rna %>%
  filtro(sexo == "Masculino") %>%
  seleccionar(gen, muestra, tejido, expresión)
```
...
rna3 <- rna %>%
  ...
  select(gen, muestra, tejido, expresión)
...
arn %>%
  mutar(expresión = log(expresión)) %>%
  seleccionar(gen, nombre_cromosoma, descripción_fenotipo, muestra, expresión) %>%
  filtrar(nombre_cromosoma = = "X" | nombre_cromosoma == "Y") %>%
  filtro(!is.na(descripción_fenotipo)) %>%
  filtro(expresión > 5)
```
...
rna %>%
  group_by(gen, tiempo) %>%
  resumen(exp_media = media(expresión))
```
...
annot1 <- read_csv(archivo = "datos/annot1.csv")
annot1
```
...
```{r}
unión_completa(rna_mini, annot1)
```
...
annot2 <- read_csv(archivo = "datos/annot2.csv")
annot2
```
...
write_csv(rna_wide, archivo = "data_output/rna_wide.csv")
```
