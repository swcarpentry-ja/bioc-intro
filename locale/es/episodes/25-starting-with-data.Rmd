...
- Describe qué es un "marco.de.datos".
- Cargue datos externos desde un archivo .csv en un marco de datos.
...
```{r, eval=TRUE, purl=TRUE}
rna <- read.table(archivo = "data/rnaseq.csv",
                  sep = ",",
...
# primer elemento en la primera columna del marco de datos (como vector)
rna[1, 1]
# primer elemento en la sexta columna (como vector)
rna [1, 6]
# primera columna del marco de datos (como un vector)
rna[, 1]
# primera columna del marco de datos (como un data.frame )
rna[1]
# primeros tres elementos en la séptima columna (como un vector)
rna[1:3, 7]
# la tercera fila del marco de datos (como un data.frame)
rna[3, ]
# equivalente a head_rna <- head(rna)
head_rna <- rna[1:6, ]
cabeza_rna
```
...
```{r, eval=FALSE, purl=TRUE}
rna[, -1] ## Todo el marco de datos, excepto la primera columna
rna[-c(7:66465), ] ## Equivalente a head(rna)
```
...
rna_200 <- rna[200, ]
## 2.
## Guardando `n_rows` para mejorar la legibilidad y reducir la duplicación
n_rows < - nrow(rna)
rna_last <- rna[n_rows, ]
## 3.
...
```{r, purl=TRUE}
dmy(pegar(x$day, x$month, x$year, sep = "-"))
```
...
```{r list0, purl=TRUE}
l <- lista (1:10, ## numérico
          letras, ## carácter
          paquetes.instalados(), ## una matriz
          autos, ## un marco.de.datos
          lista(1, 2, 3)) ## una lista
longitud(l)
str(l)
```
...
```{r, purl=TRUE}
l[[1]] ## primer elemento
l[1:2] ## una lista de longitud 2
l[1] ## una lista de longitud 1
```
...
```{r, eval=FALSE, purl=TRUE}
write.csv(rna, archivo = "data_output/my_rna.csv")
```
