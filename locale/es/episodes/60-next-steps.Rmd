...
```{r}
se <- Experimento resumido (ensayos = lista (recuentos = matriz_conteo),
                           colData = muestra_metadatos,
                           filaData = gene_metadata)
se
```
...
```{r, purl=FALSE}
rna |>
    filtro(gen %in% c("Asl", "Apod", "Cyd2d22")) |>
    filtro(tiempo!= 4) |> seleccionar(expresión )
```
...
```{r}
colData(se)$center <- rep("Universidad de Illinois", nrow(colData(se)))
colData(se)
```
...
biblioteca("tidySummarizedExperiment")
...
```{r}
opciones ("restore_SummarizedExperiment_show" = VERDADERO)
se
```
...
```{r}
opciones("restore_SummarizedExperiment_show" = FALSO)
se
```
...
se %>%
    group_by(.sample) %>%
    resumen(total_counts=sum(counts))
```
