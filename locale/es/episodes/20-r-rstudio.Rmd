...
knitr::include_graphics("fig/estructura-directorio-de-trabajo.png")
```
...
knitr::include_graphics("fig/r-empezando-cómo-debería-verse-como.png")
```
...
knitr::include_graphics("fig/gatito-prueba-cosas.jpg")
```
...
## iris es un marco de datos de ejemplo que viene con R y head() es una
## función que devuelve la primera parte del marco de datos
dput(head(iris))
```
...
guardarRDS(iris, archivo="/tmp/iris.rds")
```
...
```{r, eval=FALSE, purl=FALSE}
algunos_datos <- readRDS(file="~/Downloads/iris.rds")
```
