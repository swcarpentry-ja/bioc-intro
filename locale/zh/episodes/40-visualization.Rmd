...
ggplot(数据 = rna_fc，映射 = aes(x = time_4_vs_0，y = time_8_vs_0)) +
  geom_point()
```
...
ggplot(数据 = rna_fc，映射 = aes(x = time_4_vs_0，y = time_8_vs_0)) +
  geom_point(alpha = 0.3，aes(颜色 = gene_biotype))
...
ggplot(数据 = rna，映射 = aes(y = expression_log，x = 样本)) +
    geom_point(aes(颜色 = 时间))
```
...
ggplot(数据 = rna，
         映射 = aes(y = expression_log，x = 样本)) +
  geom_jitter(alpha = 0.2，颜色 = “tomato”) +
  geom_boxplot(alpha = 0) +
  主题(axis.text.x = element_text(angle = 90，hjust = 0.5，vjust = 0.5))
```
...
ggplot(data = rna,
         map = aes(y = expression_log,
                       x = sample)) +
  geom_jitter(alpha = 0.2, aes(颜色 = as.因子(时间))) +
  geom_boxplot(alpha = 0) +
  主题(轴.文本.x = element_text(角度 = 90, hjust = 0.5, vjust = 0.5))
```
...
ggplot(数据 = rna，
         映射 = aes(y = expression_log，x = 样本)) +
  geom_violin(aes(填充 = as.factor(时间))) +
  主题(轴.文本.x = element_text(角度 = 90，hjust = 0.5，vjust = 0.5))
```
...
ggplot(数据 = rna，
         映射 = aes(y = expression_log，x = 样本)) +
  geom_violin(aes(填充 = 性别)) +
  主题(axis.text.x = element_text(角度 = 90，hjust = 0.5，vjust = 0.5))
```
...
ggplot(数据 = mean_exp_by_time，映射 = aes(x = 时间，y = mean_exp)) +
  geom_line()
```
