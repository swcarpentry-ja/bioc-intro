...
```{r}
基因 <- 选择（rna，基因，hsapiens_homolog_associated_gene_name）
基因
```
...
```{r, purl=TRUE}
过滤器（基因，is.na（hsapiens_homolog_associated_gene_name））
```
...
rna_with_missing_values <- rna %>%
  选择（基因，样本，表达）%>%
  过滤器（基因 %in% c（“Asl”，“Apod”，“Cyp2d22”））%>%
  过滤器（样本 %in% c（“GSM2545336”，“GSM2545337”，“GSM2545338”））%>%
  安排（样本）%>%
  过滤器（！（基因 == “Cyp2d22” & 样本 != “GSM2545338”））
...
rna1 <- rna %>%
选择（基因，小鼠，表达）%>%
pivot_wider（names_from = 小鼠，values_from = 表达）
rna1
...
```{r}
 rna %>%
  过滤器（chromosome_name == “Y” | chromosome_name == “X”）%>%
  group_by（性别，chromosome_name）%>%
  总结（平均值 = 平均值（表达式））
```
...
```{r, message=FALSE}
下载文件（url = “https://carpentries-incubator.github.io/bioc-intro/data/annot1.csv”，
...
```{r, message=FALSE}
下载文件（url = “https://carpentries-incubator.github.io/bioc-intro/data/annot2.csv”，
              目标文件 = “data/annot2.csv”）
...
```{r}
full_join（rna_mini，annot2，by = c（“基因”=“external_gene_name”））
```
