...
## 样本注释
sample_metadata <- rna %>%
  选择（样本、生物体、年龄、性别、感染、菌株、时间、组织、小鼠）
...
## 基因注释
gene_metadata <- rna %>%
  选择（基因、ENTREZID、产品、ensembl_gene_id、external_synonym、
...
```{r}
colData(se)$center <- rep("伊利诺伊大学", nrow(colData(se)))
colData(se)
```
...
```{r}
选项（“restore_SummarizedExperiment_show” = TRUE）
se
```
...
```{r}
选项（“restore_SummarizedExperiment_show” = FALSE）
se
```
